...
gamma = (np.array([0,np.inf]), np.array([1.,1.]))
 
sim_0 = m.run_simulation(days = 110, m = m_0, N = 1e6, dt = dt, norm = True, gamma = gamma)
...
ax1.plot(dt * sim_0[0], sim_0[5], label = '$U_0$', color = "C0", linewidth = reference_linewidth)
...
for i in range(n_pert_runs):
    ...
    ax1.plot(dt * pertm[i][0][deviation_mask_m], pertm[i][5][deviation_mask_m], 
    ...
    #         linewidth = deviation_linewidth, color = "C2")#, label = '$I_{+5\%}$')
    ax1.plot(dt * pertp[i][0][deviation_mask_p], pertp[i][5][deviation_mask_p], 
             linewidth = deviation_linewidth, color = "C3")#, label = '$A_{+5\%}$')
