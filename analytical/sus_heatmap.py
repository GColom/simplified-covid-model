...
c0s = np.linspace(0.5, 1, 200)
ss  = np.linspace(0.2, 1.25, 200)

c0grid, sgrid = np.meshgrid(c0s, ss)

suss = np.empty(shape = (*c0s.shape,*ss.shape), dtype = float)
gridsuss = np.empty(shape = c0grid.shape)

for i in range(c0grid.shape[0]):
    for j in range(c0grid.shape[1]):
        mu = find_root(c0grid[i,j], sgrid[i,j]).root
        sus0 = sus(mu, beta, c0grid[i,j], sgrid[i,j], T_u, T_e, alpha, a)
        gridsuss[i, j] = np.abs((T_u * np.exp(-mu))/((1-alpha)*sus0)) * r
...
print(gridsuss.min(), gridsuss.max())
...
shrunk_cmap = shiftedColorMap(matplotlib.cm.RdBu_r, start=0., 
                              midpoint=0.5, 
                              stop=0.5 + 0.5 * gridsuss.max()/abs(gridsuss.min()), name='shrunk')
m = ax.pcolormesh(c0grid, sgrid, gridsuss, cmap ="plasma" )
#c = ax.contour(c0grid, sgrid, gridsuss, colors = "black", levels = [1.] ,linestyles = "solid", negative_linestyles = "solid")#, norm = norm)
#ax.clabel(c, fmt = {c.levels[0] : r"$\sigma = 1$"}, inline=True, fontsize=10)

cbar = plt.colorbar(m) 
cbar.ax.set_ylabel(r"$\sigma$", fontsize = 14)
ax.set_xlabel(r"$n_0$", fontsize = 14)
ax.set_ylabel(r"$s_0$", fontsize = 14)
plt.tight_layout()
plt.savefig("sus_heatmap.pdf", dpi = 300, transparent = True)
plt.savefig("sus_heatmap.png", dpi = 300, transparent = True)
plt.figure()
plt.title(sgrid[180, 0])
for i,s in enumerate(sgrid[::50,0]):
    plt.plot(c0s, gridsuss[50*i-1], label = fr"$s_0={s}$") 
